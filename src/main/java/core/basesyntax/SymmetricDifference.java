...
public class SymmetricDifference<T> {
    public Set<T> symmetricDifference(Set<? extends T> set1, Set<? extends T> set2) {
        Set<T> containsOfBoth = new HashSet<>(set1);
        containsOfBoth.retainAll(set2);
        Set<T> allLists = new HashSet<>(set1);
        allLists.addAll(set2);
        allLists.removeAll(containsOfBoth);
        return allLists;
    }
}
